...
class SurveyCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if (
            "expired_date" in attrs
            and attrs["expired_date"] is not None
            and attrs["expired_date"] < timezone.now()
        ):
            errors.setdefault("expired_date", []).append(
                INVALID_SURVEY_EXPIRED_FUTURE_DATE
            )

        if (
            "start_date" in attrs
            and "start_date" in attrs
            and attrs["expired_date"] is not None
            and attrs["expired_date"] is not None
        ):
    ...
    def validate(self, attrs):
        ...
        if "expired_date" in attrs and attrs["expired_date"] is not None:
            ...
            if (
                ...
                and self.context["surveyObj"].start_date >= attrs["expired_date"]
            ):
                errors.setdefault("expired_date", []).append(
                    INVALID_SURVEY_EXPIRED_FUTURE_DATE
                )
        ...
        if (
            ...
            # validate start date
            if (
                "expired_date" not in attrs
                and self.context["surveyObj"].expired_date <= attrs["start_date"]
            ):
        ...
        if "status" in attrs and attrs["status"] == "S":
            ...
            if (
                "expired_date" not in attrs
                and self.context["surveyObj"].expired_date <= timezone.now()
            ):
                errors.setdefault("expired_date", []).append(
                    INVALID_SURVEY_EXPIRED_FUTURE_DATE
                )
